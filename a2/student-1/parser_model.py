...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        cols=self.n_features * self.embed_size
        rols=self.hidden_size

        w_tensor=torch.empty(rols, cols)
        nn.init.xavier_uniform_(w_tensor)
        self.embed_to_hidden_weight=nn.Parameter(w_tensor)

        b_tensor=torch.empty(rols,)
        nn.init.uniform_(b_tensor)
        self.embed_to_hidden_bias=nn.Parameter(b_tensor)

        self.dropout = nn.Dropout(p=self.dropout_prob)

        logit_w=torch.empty(self.n_classes,self.hidden_size)
        nn.init.xavier_uniform_(logit_w)
        self.hidden_to_logits_weight=nn.Parameter(logit_w)

        logit_b=torch.empty(self.n_classes,)
        nn.init.uniform_(logit_b)
        self.hidden_to_logits_bias=nn.Parameter(logit_b)
    ...
    def embedding_lookup(self, w):
        ...
        flat_indices = w.view(-1)
        selected_embeddings = torch.index_select(self.embeddings, 0, flat_indices)
        reshaped_embeddings = selected_embeddings.view(batch_size, self.n_features, self.embed_size)
        x = reshaped_embeddings.view(batch_size, -1)

        """
        整个流程如下，这部分内容其实就是让你根据w找到对应的embeding。
        由于w是批处理的，矩阵形状是(batch_size,n_features)，所以先对其进行一维化。
        得到一维化的结果后，再embedding里面根据索引找出，得到一个二维化的答案，
        但是此时已经是(batch_size*n_features,embed_size)
        而我们需要的是(batch_size,other),所以三维化后再拍扁即可。

        这部分内容的作用是从w里提取出有用的词向量。
        """
    ...
    def forward(self, w):
        ...
        dropout_hidden=self.dropout(relu_hidden)
        logits=torch.matmul(dropout_hidden,self.hidden_to_logits_weight.T)+self.hidden_to_logits_bias
