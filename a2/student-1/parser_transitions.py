...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition=='S':
          ...
          self.dependencies.append((s0,s1))
          self.stack.pop(-2)
          ...
          self.dependencies.append((s1,s0))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
      ...
      current_steps=model.predict(current_parses)

      for current_parse, current_step in zip(current_parses, current_steps):
        current_parse.parse_step(current_step)
