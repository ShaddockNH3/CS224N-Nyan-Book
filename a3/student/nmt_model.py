...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X_conv_out = self.post_embed_cnn(X_permuted)
        X = X_conv_out.permute(2, 0, 1)
        ...
        init_decoder_hidden_combined = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_cell_combined = torch.cat((last_cell[0], last_cell[1]), dim=1)

        # 通过线性层投影，将尺寸从 2*h 压缩到 h
        init_decoder_hidden = self.h_projection(init_decoder_hidden_combined) # 形状变为 (b, h)
        init_decoder_cell = self.c_projection(init_decoder_cell_combined)   # 形状变为 (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1. 对编码器的“上下文笔记”进行一次线性投影，为后续的注意力计算做准备。
        #    这就像是把厚厚的参考书，画好重点，变成一本更易查阅的“重点笔记”。
        enc_hiddens_proj = self.att_projection(enc_hiddens)

        # 2. 查阅“目标语言大百科”，把目标句子的ID转换成词向量。
        Y = self.model_embeddings.target(target_padded)

        # 3. 按时间步（也就是一个一个词）地拆分目标句子的词向量，然后开始循环写作。
        for Y_t in torch.split(Y, 1, dim=0):
            # Y_t 的原始形状是 (1, b, e)，我们需要把它变成 (b, e)
            Y_t_squeezed = torch.squeeze(Y_t, dim=0)

            # 根据PDF，解码器的输入是“当前要写的词”和“上一笔的笔迹”的拼接。
            Ybar_t = torch.cat([Y_t_squeezed, o_prev], dim=1)

            # 把所有材料交给“单步执行官”step，让它完成这一步的思考和写作。
            # 它会返回更新后的大脑状态、这一笔的笔迹(o_t)，以及注意力分数。
        ...
        # 4. 循环结束后，把收集到的所有“笔迹”（一个列表）堆叠成一个完整的张量。
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 2. 计算注意力输出 a_t。这是 alpha_t 和原始编码器隐藏状态 enc_hiddens 的加权和。
        #    我们再次使用 bmm 来完成这个操作。
        #    - alpha_t: (b, src_len) -> (b, 1, src_len)
        #    - enc_hiddens: (b, src_len, 2h)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # 3. 拼接 a_t 和 dec_hidden，得到 U_t。
        U_t = torch.cat([a_t, dec_hidden], dim=1)

        # 4. 对 U_t 进行线性投影“蒸馏”，得到 V_t。
        V_t = self.combined_output_projection(U_t)

        # 5. 应用 Tanh 激活和 Dropout，得到最终的组合输出 O_t。
